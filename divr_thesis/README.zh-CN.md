...
`text_keys` 里的音频路径按绝对路径处理，训练或评估时必须在当前
机器上实际可访问。
...
`--test-splits all` 会把目标任务目录里的 `train.yml`、`val.yml` 和
`test.yml` 合并成一个评估集。`--eval-task-dirs` 也可以一次传入多个
任务目录，让同一个 checkpoint 依次在多个数据集上评估。
...
`results/training_summary.json` 会记录最佳 checkpoint 对应的验证 balanced accuracy、原始 accuracy 和 macro F1。`results/test_summary.json` 会记录加载的 checkpoint 对应的测试 balanced accuracy、原始 accuracy 和 macro F1。
