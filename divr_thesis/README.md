...
The audio paths in `text_keys` are treated as absolute paths and must exist
on the machine where you train or evaluate.
...
Evaluate a saved checkpoint on another dataset:

```bash
python src/__main__.py \
  --evaluate \
  --source-run-dir .cache/runs/femh_1_test_wav2vec_large_concatenation_none_20260504_050445 \
  --eval-task-dirs tasks/voiced_1_test \
  --test-splits all \
  --checkpoint-name best.pt \
  --device auto
```

Use `--test-splits all` to merge the target task's `train.yml`, `val.yml`,
and `test.yml` into one evaluation set. You can pass multiple folders to
`--eval-task-dirs` to reuse the same checkpoint across several datasets.
...
- records validation balanced accuracy, raw accuracy, and macro F1 in `results/history.csv`
- saves `best.pt` when validation balanced accuracy improves
...
`results/training_summary.json` includes the best checkpoint epoch plus validation balanced accuracy, raw accuracy, and macro F1. `results/test_summary.json` includes the loaded checkpoint epoch plus test balanced accuracy, raw accuracy, and macro F1.
