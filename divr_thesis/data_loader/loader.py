...
class TaskDataModule:
    def __init__(
        ...
        self.test_split_names = self._normalize_split_names(
            test_split_names
        )
        ...
        self.test_records = self._load_split("test")
        self._split_records = {
        ...
        self.train_dataset = self._build_dataset(self.train_records)
        self.val_dataset = self._build_dataset(self.val_records)
        self.test_dataset = self._build_dataset(
            self._records_for_splits(self.test_split_names)
        )
    ...
    def loader_for_splits(
        self,
        split_names: Sequence[str] | None,
        shuffle: bool = False,
    ) -> TorchDataLoader[Batch]:
        return self._make_loader(
            self._build_dataset(self._records_for_splits(split_names)),
            shuffle=shuffle,
        )
    ...
    def _build_dataset(self, records: Sequence[TaskRecord]) -> TaskDataset:
        return TaskDataset(
            records=records,
    ...
    def _records_for_splits(
        ...
        normalized_split_names = self._normalize_split_names(split_names)
        records: list[TaskRecord] = []
        for split_name in normalized_split_names:
            records.extend(self._split_records[split_name])
        return records

    def _normalize_split_names(
        self,
        split_names: Sequence[str] | None,
    ) -> tuple[str, ...]:
        ...
        if not normalized:
            raise ValueError("test_split_names cannot be empty")
        if any(split_name == "all" for split_name in normalized):
            return ("train", "val", "test")
        deduplicated = tuple(dict.fromkeys(normalized))
        valid_split_names = {"train", "val", "test"}
        invalid = [
            split_name
            for split_name in deduplicated
            if split_name not in valid_split_names
        ]
        if invalid:
            raise ValueError(
                "Unsupported split names: " + ", ".join(invalid)
            )
        return deduplicated
